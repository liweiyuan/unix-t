...
int main() {
  ...
  if (sem_avail == SEM_FAILED) {
    perror(
        "sem_open failed for SEM_AVAIL"); // 打开信号量失败，打印错误信息并退出
    exit(EXIT_FAILURE);
  }
  ...
  if (sem_done == SEM_FAILED) {
    perror(
        "sem_open failed for SEM_DONE"); // 打开信号量失败，打印错误信息并退出
    exit(EXIT_FAILURE);
  }
  ...
  for (int i = 0; i < 4; i++) {
    ...
    if (shm_stat.st_size > header->capacity) {
      ...
      if (header == MAP_FAILED) {
        perror(
            "mmap failed after resizing"); // 如果 mmap 失败，打印错误信息并退出
        exit(EXIT_FAILURE);
      }
    }
    ...
    if (header->ready_flag) {
      ...
      memcpy(buf, header->data,
             header->data_size); // 从共享内存中复制数据到缓冲区
